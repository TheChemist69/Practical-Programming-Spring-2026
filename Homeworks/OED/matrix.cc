...
// Construct an nxm zero matrix (m columns, each of length n)
...
// Element access: A[i,j] -> cols[j][i]  (column-major storage)
...
// Matrix-vector: (Av)_i = sum_j( A_{ij} v_j )
// Inner loop over j (columns) for column-major locality.
...
// Loop order j-k-i: the inner (i) loop streams down a column
// of A and a column of C, both contiguous in column-major layout.
