...
// Writes a separator line in the report.
void write_rule(std::ostream& out) {
  out << "# ------------------------------------------------------------\n";
}

// Writes a titled section header in the report.
...
// Writes a key-value pair with an integer value.
void write_kv(std::ostream& out, const char* key, int value) {
  out << "# " << std::left << std::setw(24) << key << ": " << value << "\n";
  out << std::right;
}

// Writes a key-value pair with a floating-point value.
void write_kv(std::ostream& out, const char* key, double value) {
  out << "# " << std::left << std::setw(24) << key << ": " << value << "\n";
  out << std::right;
}

// Writes a labeled interval.
...
int main() {
  ...
  write_title(out, "Practical Programming - Homework: Artificial Neural Networks");
  ...
  write_title(out, "Exercise 1 - Interpolation of g(x)");
  write_kv(out, "Target", "g(x)=cos(5x-1)exp(-x^2)");
  write_kv(out, "Activation", act.name.c_str());
  write_kv(out, "Hidden neurons", neurons);
  write_interval(out, "Sample range", x_min, x_max);
  write_kv(out, "Samples", samples);
  write_kv(out, "Init log(b)", train_opts.init_log_b);
  write_kv(out, "Max iters", train_opts.minimize.max_iters);
  write_kv(out, "Grad tol", train_opts.minimize.grad_tol);
  write_kv(out, "Step init", train_opts.minimize.step_init);
  write_kv(out, "Step min", train_opts.minimize.step_min);
  write_kv(out, "Iterations", report.minimize.iters);
  write_kv(out, "Final cost", report.minimize.final_cost);
  write_kv(out, "Final grad norm", report.minimize.final_grad_norm);
  write_kv(out, "RMS error", rms_fit);
  out << "#\n";
  ...
  write_title(out, "Exercise 2 - Derivatives and Antiderivative");
  write_kv(out, "Antiderivative", "Integral from x0 to x, x0 = first sample");
  write_kv(out, "RMS g'(x)", rms_d1);
  write_kv(out, "RMS g''(x)", rms_d2);
  out << "#\n";
  ...
  write_title(out, "Exercise 3 - ODE Training");
  write_kv(out, "Equation", "y'' + y = 0");
  write_kv(out, "Boundary", "y(0)=0, y'(0)=1");
  write_interval(out, "Interval", ode_opts.a, ode_opts.b);
  write_kv(out, "c", ode_opts.c);
  write_kv(out, "alpha", ode_opts.alpha);
  write_kv(out, "beta", ode_opts.beta);
  write_kv(out, "Samples", ode_opts.samples);
  write_kv(out, "Activation", act.name.c_str());
  write_kv(out, "Hidden neurons", neurons);
  write_kv(out, "Iterations", ode_report.minimize.iters);
  write_kv(out, "Final cost", ode_report.minimize.final_cost);
  write_kv(out, "Final grad norm", ode_report.minimize.final_grad_norm);
  write_kv(out, "RMS error vs sin(x)", rms_ode);
  out << "#\n";
