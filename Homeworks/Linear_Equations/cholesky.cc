...
cholesky::cholesky(const pp::matrix& A) {
    int n = A.rows();
    if (n != A.columns()) {
...
pp::vector cholesky::forward_substitute(const pp::vector& b) const {
    int n = L.rows();
    pp::vector y(n);
    for (int i = 0; i < n; i++) {
        double sum = b[i];
...
pp::vector cholesky::back_substitute(const pp::vector& y) const {
    int n = L.rows();
    pp::vector x(n);
    for (int i = n - 1; i >= 0; i--) {
        double sum = y[i];
...
// Solve Ax = b  =>  LL^T x = b
pp::vector cholesky::solve(const pp::vector& b) const {
...
pp::matrix cholesky::inverse() const {
    int n = L.rows();
    pp::matrix B(n, n);
    for (int j = 0; j < n; j++) {
        pp::vector ej(n);
